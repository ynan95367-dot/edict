...
def handle_create_task(title, org='待办', official='太子', priority='normal', template_id='', params=None, target_dept=''):
    """从看板创建新任务（圣旨模板下旨）。"""
    ...
    # 正确流程起点：皇上 -> 太子分拣
    # target_dept 记录模板建议的最终执行部门（派发参考）
    initial_org = '太子'
    new_task = {
        'id': task_id,
        'title': title,
...
def handle_review_action(task_id, action, comment=''):
    ...
    if not task:
        return {'ok': False, 'error': f'任务 {task_id} 不存在'}
    if task.get('state') not in ('Review',):
        return {'ok': False, 'error': f'任务 {task_id} 当前状态为 {task.get("state")}，无法批阅'}
    ...
    if action == 'approve':
        ...
        if total > 0 and completed < total:
            return {'ok': False, 'error': f'子任务尚未全部完成（{completed}/{total}），不能直接完成'}
        task['state'] = 'Done'
        task['now'] = '验证通过，任务完成'
        remark = f'✅ 验证通过：{comment or "验证通过"}'
        to_dept = '用户'
    elif action == 'reject':
        round_num = (task.get('review_round') or 0) + 1
        task['review_round'] = round_num
        task['state'] = 'Zhongshu'
        task['now'] = f'驳回修订（第{round_num}轮）'
    ...
    task.setdefault('flow_log', []).append({
        'at': now_iso(),
        'from': '刑部' if task.get('state') not in ('Done',) else '用户',
        'to': to_dept,
        'remark': remark
    })
...
_AGENT_DEPTS = [
    {'id':'taizi',   'label':'太子',  'emoji':'🤴', 'role':'编排者',   'rank':'储君'},
    {'id':'hubu',    'label':'户部',  'emoji':'💰', 'role':'提出者', 'rank':'正二品'},
    {'id':'libu',    'label':'礼部',  'emoji':'📝', 'role':'建单者', 'rank':'正二品'},
    {'id':'bingbu',  'label':'兵部',  'emoji':'⚔️', 'role':'经办者', 'rank':'正二品'},
    {'id':'xingbu',  'label':'刑部',  'emoji':'⚖️', 'role':'验证者', 'rank':'正二品'},
    {'id':'gongbu',  'label':'工部',  'emoji':'🔧', 'role':'维护者', 'rank':'正二品'},
    {'id':'libu_hr', 'label':'吏部',  'emoji':'👔', 'role':'跟踪者', 'rank':'正二品'},
    {'id':'jira_bridge',       'label':'JIRA交互', 'emoji':'🔗', 'role':'API网关',   'rank':'从三品'},
    {'id':'version_integrator','label':'版本集成', 'emoji':'📊', 'role':'版本官',   'rank':'从三品'},
    {'id':'report_generator',  'label':'报表生成', 'emoji':'📈', 'role':'史官',     'rank':'从三品'},
]
...
_ORG_AGENT_MAP = {
    ...
    '太子': 'taizi', 'JIRA交互': 'jira_bridge',
    '版本集成': 'version_integrator', '报表生成': 'report_generator',
}
...
# 状态推进顺序（手动推进用）
_STATE_FLOW = {
    'Pending':  ('Taizi', '用户', '太子', '新任务待太子编排'),
    'Taizi':    ('Pending', '太子', '', '太子已分发到对应部门'),
    'Doing':    ('Review', '兵部', '刑部', '经办完成，转刑部验证'),
    'Review':   ('Done', '刑部', '太子', '验证通过，回奏太子'),
}
_STATE_LABELS = {
    'Pending': '待编排', 'Taizi': '分拣中', 'Doing': '执行中',
    'Review': '验证中', 'Done': '完成', 'Blocked': '阻塞', 'Cancelled': '已取消',
}
...
def main():
    parser = argparse.ArgumentParser(description='JIRA 管理助手看板服务器')
    ...
    log.info(f'JIRA 助手看板启动 → http://{args.host}:{args.port}')
    print(f'   按 Ctrl+C 停止')
