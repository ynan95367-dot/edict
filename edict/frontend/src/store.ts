...
export const DEPT_COLOR: Record<string, string> = {
  '太子': '#d9ae5f', '中书省': '#b692f6', '门下省': '#59d0c2', '尚书省': '#58d68d',
  '礼部': '#d9ae5f', '户部': '#f59e72', '兵部': '#ef6f64', '刑部': '#e06b6b',
  '工部': '#75bdff', '吏部': '#c08df4', '皇上': '#f6d365', '回奏': '#58d68d',
};
...
export type TabKey =
  | 'command' | 'edicts' | 'monitor' | 'officials' | 'models'
  | 'skills' | 'sessions' | 'outputs' | 'memorials' | 'templates' | 'morning' | 'court';

export const TAB_DEFS: { key: TabKey; label: string; icon: string }[] = [
  { key: 'command',   label: '下达指令', icon: '⌘' },
...
interface AppStore {
  ...
  runtimeOutbox: RuntimeOutboxHealth | null;
  ...
  loadRuntimeOutbox: () => Promise<void>;
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  loadRuntimeOutbox: async () => {
    try {
      const data = await api.runtimeOutbox();
      set({ runtimeOutbox: data });
    } catch {
      set({ runtimeOutbox: null });
    }
  },
  ...
  loadAll: async () => {
    const s = get();
    await Promise.all([s.loadLive(), s.loadRuntimeOutbox()]);
