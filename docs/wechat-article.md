...
多 Agent 系统最怕的，不是某个任务失败，而是**看起来还活着，其实已经半死不活**。

比如 OpenCode Server 的 `/doc` 和 `/agent` 接口可能还能返回 200，但底层 session registry 已经陈旧了。表面上你以为系统健康，真正派发任务时却会报一句：`Session not found`。这种问题最烦，因为它不是彻底挂掉，而是**假活着**。

所以我做了两层探针。

第一层是**基础存活探针**：检查 HTTP 入口是否还能响应、核心 Agent 名单是否还在。它回答的问题是："服务还在不在？"

第二层是**会话级探针**：不只看接口通不通，而是真实地创建一个临时 session，再立刻删除。只有这一步成功，我才认为派发链路真的可用。它回答的问题是："现在马上下旨，系统接不接得住？"

这两层探针背后的逻辑很简单：**监控不能只看心电图，得看病人能不能下床走两步。**

如果会话探针失败，系统不会傻等人工处理，而是立即进入自愈流程：

- 先记录派发失败原因，避免把故障吞掉
- 自动重启 OpenCode Server，清理失效的 session registry
- 重启后再次做 probe，确认不是"重启了个寂寞"
- 验证通过后再重试派发，避免把坏状态继续扩散

如果问题不是运行时故障，而是任务本身长时间没有推进，太子调度还会接管第二层守护：**停滞扫描 → 自动重试 → 升级协调 → 自动回滚 → 挂起等待人工介入**。

我给它设了一条很像值班 SRE 的处理链：任务超过阈值没进展，先自动重试；重试还不动，就升级到门下省、尚书省协调；如果连续停滞，直接回滚到上一个稳定节点；回滚多次仍失败，就自动挂起并明确写上阻塞原因。

这套机制的意义，不是为了让系统"永不失败"。那不现实。它真正解决的是另一件事：**失败发生时，系统能不能自己判断、自己止血、自己恢复到可操作状态。**

对单机脚本来说，失败了重跑就行；但对多 Agent 协作系统来说，失败常常发生在链路之间、状态之间、责任边界之间。你缺的不是一个 `retry=True`，而是一整套"发现异常—缩小影响—恢复执行—保留证据"的治理机制。

这也是为什么我后来越来越觉得：多 Agent 的稳定性问题，本质上不是工程细节，而是**调度制度设计**。
